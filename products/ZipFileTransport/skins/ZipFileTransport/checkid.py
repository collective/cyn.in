...
objects as files and images that supply an alternative id when an id is
autogenerated. If you say "There is already an item with this name in this folder"
for an image that has the Name field populated with an autogenerated id, it can
cause some confusion, since the real problem is the name of the image file name,
not in the name of the autogenerated id.
"""
...
if plone_utils is not None:
    ...
    if len(bad_chars) > 0:
        return _('characters', '%s is not a legal name. The following characters are invalid: %s.') %(id, ''.join(bad_chars) )
...
if portal_catalog is not None:
    try:
        if id in portal_catalog.indexes():
            return _('reserved', '%s is reserved.') %(id)
    except Unauthorized:
...
if checkForCollision:
    ...
    if contained_by is None:
        try:
            ...
            return _('','')# nothing we can do

    if hasattr(contained_by, 'contentIds'):
        try:
            if id in contained_by.contentIds():
                return _('existing', 'There is already an item named %s in this folder.') %(id)
        except Unauthorized:
