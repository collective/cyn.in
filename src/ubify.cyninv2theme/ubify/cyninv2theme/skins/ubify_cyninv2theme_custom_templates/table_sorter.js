...
function sort() {
    ...
    jq(this).children('img.sortdirection').attr('src', portal_url +
        (reverse ? '/icon-arrow_down.png' : '/icon-arrow_up.png'));
...
jq(function() {
    ...
    var blankarrow = jq('<img>')
        .attr('src', portal_url + '/arrowBlank.gif')
        .attr('width', 16).attr('height', 16).addClass('sortdirection');
    ...
    jq('table.listing:not(.nosort) thead th:not(.nosort)')
        ...
        .click(sort)
        .slice(0, 1)
        .find('img.sortdirection').attr('src', portal_url + '/icon-arrow_up.png');
});
