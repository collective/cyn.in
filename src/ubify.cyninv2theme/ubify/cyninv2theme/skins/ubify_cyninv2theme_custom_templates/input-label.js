...
var ploneInputLabel = {
    ...
    submit: function(e) {
		...
		jq('textarea[title].inputLabelActive').filter(function() {
            return jq(this).val() == this.title;
        }).val('').removeClass('inputLabelActive');
    }
};

function triggerInputLabel(){
    jq(function() {
	...
	jq('input[title].inputLabel').each(function() {
	    ...
	    jq('textarea[title].inputLabel').each(function() {
	    jq(this).focus(ploneInputLabel.focus).blur(ploneInputLabel.blur);
	    if (!jq(this).val())
		jq(this).val(this.title)
		       .removeClass('inputLabel').addClass('inputLabelActive');
...
function triggerResetInputLabel(selector){
    selector.addClass('inputLabelActive').val(selector.attr('title'));
}
triggerInputLabel();