...
class TopbarMyButton(BrowserView):
    """Contains backend code for the userinfo tooltip
    """

    template = ViewPageTemplateFile('topbarmybutton.pt')
    allowed_types = spacesdefaultaddablenonfolderishtypes + ('StatuslogItem',)
    ...
    def isUserSpaceExists(self,mtool,userid):
        result = False
        try:
            homeFolder = mtool.getHomeFolder(userid)
            objpubspace = getattr(homeFolder,'public')
            if objpubspace <> None:
                result = True
    ...
    def getportaltypes(self):        
        returnvalue = ""
        for pt in self.allowed_types:
            returnvalue = returnvalue + 'portal_type:list=' + pt + "&"
