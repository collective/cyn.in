...
class StackerView(BrowserView):
    ...
    def getSiteInfo(self):
        ...
        if hasattr(site_prop,'base_rss_url') and site_prop.base_rss_url <> '':
            ...
            baseurl = site.absolute_url()
            
        if hasattr(site_prop,'allow_discussion_title') and site_prop.allow_discussion_title == True:
            discussion_has_title = True
        else:
            discussion_has_title = False

        if hasattr(site_prop,'desktop_login_url') and site_prop.desktop_login_url <> '':
            desktop_login_url = site_prop.desktop_login_url
        else:
            desktop_login_url = site.absolute_url()
            
        if hasattr(site_prop,'desktop_donot_autologin') and site_prop.desktop_donot_autologin <> True:
            desktop_donot_autologin = False
        else:
            desktop_donot_autologin = True

        siteinfo['title'] = site.Title()
        siteinfo['description'] = site.Description()
        siteinfo['baseurl'] = baseurl
        siteinfo['discussion_has_title'] = discussion_has_title
        siteinfo['xmlrpc_root'] = desktop_login_url
        siteinfo['xmlrpc_autologin_url_launches'] = desktop_donot_autologin
    ...
    def setItem(self,obj,commentcount):
        ...
        if obj <> None:
            item = {
                    ...
                    'lastchangedate':obj.lastchangedate or obj.modified(),
                    'lastchangeaction':obj.lastchangeaction or "modified",
                    'lastchangeperformer':obj.lastchangeperformer or obj.creator(),
    ...
    def getUserRecentItems(self,username="", maxitemcount=10):
        ...
        from ubify.cyninv2theme.portlets.statistics import getRecentContributionForUser
        
        if username == "":
            username = self.current_user()
        resbrains,more,moreurl = getRecentContributionForUser(self.context,user=username,records=maxitemcount, types_to_include=self.stackercontenttypes)
        
        fullcount = more
        outlist = [self.setItem(item.getObject(),-1) for item in resbrains]
        return {'itemcount':fullcount,'itemlist':outlist}
    ...
    def getTypeInfo(self,typename):
        ...
        if object_typeobj is not None:
            ...
            return {
                    'typename':typename,
                    'typetitle':typetitle,
                    'typeiconurl':self.context.portal_url.getPortalObject().absolute_url() + '/' + typeiconname
                    }
    
    def list_portal_types(self):
        """Returns a list of all known content types, as the standard TypeInfo dict"""
        import operator
        typelist = self.getTypesByNames(self.stackercontenttypes)
        typelist.sort(key=operator.itemgetter('typename'))
        return typelist
    ...
    def getUserInfo(self,userid=''):
		...
		mobile = user.getProperty('phone_mobile','')
		designation = user.getProperty('designation','')
		officephone = user.getProperty('phone_office','')
		homephone = user.getProperty('phone_home','')
		portrait = pm.getPersonalPortrait(userid).absolute_url()
		hashomefolder = False
		try:
		    hashomefolder = pm.getHomeFolder(userid) <> None
		except AttributeError:
		    hashomefolder = False
		statusmessagedata = {}
		outlist = self.getStatusMessages(userid,1,1)
		if outlist['itemcount'] > 0 and outlist.has_key('itemlist') and len(outlist['itemlist']) > 0:
		    messageitem = outlist['itemlist'][0]
		    statusmessagedata['message'] = messageitem['title']
		    statusmessagedata['created'] = messageitem['created']
		    statusmessagedata['commentcount'] = messageitem['commentcount']
		...
		return {
		    ...
		    'statusmessage_info': statusmessagedata
		}
	    else:
    ...
    def getSpaces(self,parentUID=None):
        ...
        query = {'path': {'query': "/".join(p_url.getPortalObject().getPhysicalPath()),'depth':1}, 'portal_type': ['ContentRoot']}
        resbrains = portal_catalog(query)
        
        if len(resbrains) == 1:
	    rootbrain = resbrains[0]
            rootobj = resbrains[0].getObject()
                ...
                if len(results) > 0:
                    for o in results:
                        ...
                        itemresult = {
                                        'uid':o.UID,
                                        'id':o.id,
                                        't':o.pretty_title_or_id(),
                                        'd':o.Description,
					'dt':o.disallowedtypes,
                                        'c': [],
                                     }
                        getChildren(fullobj,catalog,itemresult['c'])
        ...
        for obj in resbrains:
            itemresult = {
                            'uid':obj.UID,
                            'id':obj.id,
                            't':obj.pretty_title_or_id(),
                            'd':obj.Description,
			    'dt':obj.disallowedtypes,
                            'c': [],
                         }
            getChildren(obj.getObject(),portal_catalog,itemresult['c'])

            outlist.append(itemresult)
        rootresult =  {'uid':rootobj.UID(),
                'id':rootobj.id,
                't':rootobj.pretty_title_or_id(),
                'd':rootobj.Description(),
		'dt':rootbrain.disallowedtypes,
                'c':outlist,
                }
        return rootresult
    ...
    def addDiscussion(self,discussiontext,tags=None,contextUID=None,title=""):
        ...
        """
        from ubify.cyninv2theme import addDiscussion as addCyninDiscussion
        if contextUID is not None:
            cat = getToolByName(self.context, 'uid_catalog', None)
	    resbrains = cat.searchResults(UID=contextUID)
	    if len(resbrains) > 0:
		addatcontext = resbrains[0].getObject()
	else:
	    addatcontext = getattr(self.context,"home",None)
	if addatcontext is not None:
	    newdisc = addCyninDiscussion(self.context.portal_url.getPortalObject(),discussiontext,tags,addatcontext)
	    return newdisc.absolute_url()
	else:
	    raise "Error: Could not ascertain add context."
	...
	if cat <> None:
	    alltags = cat.uniqueValuesFor('Subject')
	return alltags
