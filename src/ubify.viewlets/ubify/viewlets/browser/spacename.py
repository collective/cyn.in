...
class SpaceNameViewlet(ViewletBase):
    render = ViewPageTemplateFile('space_name.pt')
    ...
    def update(self):
        ...
        if new_object <> None:
            ...
            if new_object.space_icon <> "":
                self.space_icon = cportal_url + "/spaceicons/" + new_object.space_icon
            else:
                self.space_icon = default_space_icon
            self.space_url = new_object.absolute_url()
            self.has_space_icon = True
            self.space_name = new_object.title
            portal = portal_state.portal()
            portal_catalog = portal.portal_catalog
            path =  '/'.join(new_object.getPhysicalPath())
            res=portal_catalog.searchResults({'path':{'query':path,'depth':-1,'sort_on':'modified','sort_order':'descending','sort_limit':1}})
            if len(res)>0:
                self.has_modified_info = True
                obj = res[0].getObject()
                self.modified= obj.toLocalizedTime(obj.modified(),False)
            else:
                self.has_modified_info = False
        else:
            self.site_title = portal_state.portal_title()
            self.has_site_title = True
            self.site_url = portal_state.portal_url()
            self.render = ViewPageTemplateFile('sitetitle.pt')
