...
class MyAvatar(ViewletBase):
    render = ViewPageTemplateFile('myavatar.pt')
    imgtag32 = ""
    ...
    def update(self):
        ...
        if not self.anonymous:
            ...
            ##portraitstring = str(portrait.data)
            ##img = Image.open(StringIO(portraitstring))
            ##img.thumbnail((32,32),Image.ANTIALIAS)
            self.portraiturl = portrait.absolute_url()
            #import pdb; pdb.set_trace()
            try:
                self.imgtag32 = portrait.tag(height=32,xscale = portrait.width / portrait.height)
            except TypeError:
                pass
