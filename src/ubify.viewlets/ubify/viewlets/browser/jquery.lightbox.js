			...
			{	// Create image
				...
				{	// Already a image, so copy over values
					...
					image.index = obj.index || image.index;
                                        image.targetURL = obj.targetURL || image.targetURL;
				}
				else if ( obj.tagName )
				{	// We are an element
					obj = $(obj);
					if ( obj.attr('src') || obj.attr('href') )
					{
						...
						image.color = obj.css('backgroundColor');
                                                image.targetURL = obj.attr('targetURL') || image.attr('targetURL');
		...
		showImage: function ( image, step )
		{
			...
			switch ( step )
			{
				...
				case 4:
					...
					{	$title.attr('href', this.download_link ? image.targetURL : '');	}
					delete $title;
