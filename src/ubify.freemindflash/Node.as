...
class Node {
	...
	public static var defaultWordWrap:Number=200;
	...
	public function drawAroundNode(colorNoSel,alpha,isSelected){
		...
		// Style==2 Bubble
		if  (style==2){
			round_rectangle(n._width +10, n._height,colorNoSel,alpha,cf);
			...
			circle(n._width+14, n._height+4,colorNoSel,alpha,cf);
		...
		if (folded==true){
		    drawCircle(n._width+10,n._height,cf);
		}
	}
	...
	function posTextImag(initX,initY){
		if (ref_mc.node_txt.node_image!=undefined){
				ref_mc.node_txt.node_image._y += 3
				ref_mc.node_txt.node_image._x=initX + 5;
				initX+=ref_mc.node_txt.node_image._width + 5;
			}
	...
	public function genShadow(){
		delShadow();
		if(style==2 && ref_mc.node_txt._width>0){
