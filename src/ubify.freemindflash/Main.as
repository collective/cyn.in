...
class Main {
		...
		static function redefineRightMenu(){
			...
			mycm.onSelect = Main.copyInfoNodeOver;
			//var copy=new ContextMenuItem("move back",Main.backward);
			//mycm.customItems.push(copy);
			//copy=new ContextMenuItem("move forward",Main.forward);
			//mycm.customItems.push(copy);
			var copy=new ContextMenuItem("copy to clipboard",Main.getNodeText);
			mycm.customItems.push(copy);
			copy=new ContextMenuItem("unfold all from Node",Main.unfoldAllFromNode);
			mycm.customItems.push(copy);
			copy=new ContextMenuItem("fold all from Node",Main.foldAllFromNode);
			mycm.customItems.push(copy);
			//if(Browser.flashVersion>7){
			//	copy=new ContextMenuItem("gen shots for linked maps",Main.genShotsForLinkedMaps);
			//	mycm.customItems.push(copy);
			//}
			//copy=new ContextMenuItem("FREEMIND BROWSER v1.0b",Main.nada);
			//mycm.customItems.push(copy);
			mycm.hideBuiltInItems();
			_root.menu = mycm;
		}
